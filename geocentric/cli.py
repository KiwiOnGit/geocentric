...
def ensure_entrypoint_on_path() -> None:
    import stat
    script_path = Path(sys.argv[0]).resolve() if sys.argv and sys.argv[0] else None
    if not script_path or not script_path.exists():
        script_path = Path(__file__).parent.parent / "bin" / "geocentric"
    if not script_path or not script_path.exists():
        return
    ...
    if os.name == "nt":
        try:
            ...
            if script_dir not in current_path:
                subprocess.run(["setx", "PATH", f"{current_path};{script_dir}"], check=False, capture_output=True, text=True)
        except Exception:
            pass
        return

    # macOS/Linux: try symlink first, fall back to PATH in rc file
    try:
        target = Path("/usr/local/bin/geocentric")
    ...
    # Fallback: Add to PATH in shell rc files
    for shell_rc_name in [".zshrc", ".bashrc"]:
        shell_rc = Path.home() / shell_rc_name
        try:
            if not shell_rc.exists():
                continue
            content = shell_rc.read_text(encoding="utf-8", errors="ignore")
            export_line = f'export PATH="{script_path.parent}:$PATH"'
            if export_line not in content:
                with open(shell_rc, "a", encoding="utf-8") as fh:
                    fh.write(f"\n# Geocentric CLI entry point\n{export_line}\n")
                return
        except Exception:
            pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-continue", "--continue", dest="continue_chat", metavar="CHAT_ID", help="Resume a saved conversation by chat ID")
    p.add_argument("-y", "--force", action="store_true", help="Auto-approve tool actions and skip confirmation prompts")
    ...
    sv.add_argument("--non-interactive", action="store_true", help="Bypass interactive prompts and use default selections")
    sv.add_argument("--thinking-model", default=None, help="Force a specific thinking model")
    sv.add_argument("--instant-model", default=None, help="Force a specific instant model")
    ...
    ix = sub.add_parser("interactive", aliases=["cli", "chat"], help="Interactive terminal CLI with /commands (Claude Code / Cursor style)")
    ix.add_argument("--server", default=None, help="Geocentric server URL (default http://127.0.0.1:8000 or GEOCENTRIC_SERVER env)")
    ix.add_argument("--model", default=None, help="Model id to use")
    ix.add_argument("--mode", choices=["thinking", "instant"], default="thinking", help="Thinking agentic or instant mode")
    ix.add_argument("--token", default=None, help="Auth token for workspace agent mode")
    ix.add_argument("--no-agent", action="store_true", help="Disable workspace agent mode")
    ix.add_argument("-y", "--force", action="store_true", help="Auto-approve tool actions and skip confirmation prompts")

    cn = sub.add_parser("connect", help="Connect this terminal CLI to a remote Geocentric server hosting models")
    cn.add_argument("--server", required=True, help="Remote server URL, e.g. http://192.168.1.10:8000")
    cn.add_argument("--model", default=None, help="Model id to use")
    cn.add_argument("--mode", choices=["thinking", "instant"], default="thinking")
    cn.add_argument("--token", default=None, help="Auth token if required")
    cn.add_argument("--no-agent", action="store_true", help="Disable workspace agent mode")
...
def run_remote_info(port: int = 8000) -> None:
    ...
    url = f"http://{ip}:{port}"
    print("=" * 72)
    print("  Geocentric Code — Remote access over local WiFi")
    print("=" * 72)
    print()
    print("  STEP 1 — Start server on this Mac:")
    print("    ./run.sh  →  choose [2] CLI or [1] GUI")
    print()
    print("  STEP 2 — Connect from ANY device (Python 3 only, no install):")
    print(f"    curl -s {url}/api/cli/client -o geocentric-code.py")
    print(f"    python geocentric-code.py --server {url}")
    print()
    print("  Windows CMD / PowerShell — do NOT pipe curl into python (stdin breaks).")
    print("  Save the file first, then run it as shown above.")
    print()
    print("  Or SSH into this Mac, then connect to local server:")
    print(f"    ssh {user}@{ip}")
    print(f"    curl -s http://127.0.0.1:{port}/api/cli/client -o geocentric-code.py")
    print(f"    python geocentric-code.py --server http://127.0.0.1:{port}")
    print()
    print("  Full install on this Mac only (optional):")
    print(f"    cd {Path.cwd()}")
    print(f"    .venv/bin/python -m geocentric.cli connect --server http://127.0.0.1:{port}")
    print()
    print("  Enable System Settings → General → Sharing → Remote Login for SSH.")
    print("=" * 72)
...
def _serve(args: argparse.Namespace) -> None:
    ...
    if ollama_models:
        ...
        if args.thinking_model:
            selected_thinking_model = args.thinking_model
        ...
        # Prompt Instant
        if args.instant_model:
            selected_instant_model = args.instant_model
        elif is_non_interactive:
    ...
    print("  - Public tunnel URL:   requesting...")
    print("=" * 80)
    ...
    # Start secure localtunnel
    def _start_localtunnel():
        import time
        import socket
        # Wait for the local port to open to avoid ConnectionRefusedError crash
        port_open = False
        ...
        if not port_open:
            print(f"  - Public tunnel URL:   failed to start localtunnel because port {args.port} did not open in time.")
            return
        ...
        try:
            ...
            for attempt in range(5):
                try:
                    localtunnel_url = tunnel_obj.get_url("")
                    if localtunnel_url:
                        break
                except Exception:
                    time.sleep(1.0)
            ...
            if localtunnel_url:
                print(f"  - Public tunnel URL:   {localtunnel_url}")
                tunnel_obj.create_tunnel(args.port, "127.0.0.1")
            else:
                print("  - Public tunnel URL:   failed to request URL from localtunnel.me (server offline or no internet)")
        except ImportError:
            print("  - Public tunnel URL:   unavailable (py-localtunnel package not installed in environment)")
        except Exception as lt_err:
            print(f"  - LocalTunnel failed: {lt_err}")

    import threading
    threading.Thread(target=_start_localtunnel, daemon=True).start()
...
def main() -> None:
    ...
    if args.cmd == "train-tokenizer":
        ...
        _serve(args)
    elif args.cmd in {"interactive", "cli", "chat"}:
        from geocentric.interactive_cli import run_interactive_cli
        ...
        run_interactive_cli(
            ...
            connect_only=False,
            continue_chat=args.continue_chat,
            force=args.force,
        )
